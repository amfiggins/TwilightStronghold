...
## 2024-04-12 - DoS Vulnerability via Yielding RemoteFunctions
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` yielded its thread using a `while` loop containing `task.wait(0.1)` for up to 5 seconds when data was unavailable. Because it lacked a rate limit, a malicious client could rapidly and repeatedly invoke this RemoteFunction, causing the server to spawn and suspend an unbounded number of Luau threads, leading to Thread Exhaustion Denial-of-Service (DoS).
**Learning:** Yielding operations (like polling loops or `GetAsync` calls) within RemoteFunctions must always be protected by strict, non-yielding rate limits *before* the yield occurs. If a client can trigger a long-running server thread instantly, they can crash the server or cause massive lag.
**Prevention:** Always implement an immediate `os.clock()` cooldown check (returning `false` or throwing an error) at the very start of any `OnServerInvoke` callback that performs asynchronous or yielding operations.
