...
## 2024-04-23 - Thread Exhaustion DoS via RemoteFunction Polling
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` allows clients to invoke the server without any rate limiting. An attacker can repeatedly invoke this function, causing the server to spawn many polling loops (`while not data ... do task.wait(0.1) end`), leading to thread exhaustion and a Denial of Service (DoS).
**Learning:** RemoteFunctions (`OnServerInvoke`) can easily become attack vectors if they lack explicit, non-yielding rate limits immediately upon invocation, especially when the server-side logic yields or polls.
**Prevention:** Implement strict rate limits (e.g., using `os.clock()`) immediately inside `OnServerInvoke` callbacks before any yielding operations or complex logic occur, and ensure proper cleanup of tracking tables to prevent memory leaks.
