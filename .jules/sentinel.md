...
## 2024-05-20 - RemoteFunction DoS Thread Exhaustion
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` was susceptible to Denial of Service (DoS) attacks. Because it yields (using `task.wait` to poll for data), a malicious client could rapidly spam `InvokeServer()`, spinning up thousands of yielding threads and quickly exhausting server memory and processing power.
**Learning:** Any RemoteFunction that can yield before returning must have strict server-side rate limiting. Without it, the server will spawn a new Lua thread for every incoming request, which remains alive until the yield completes.
**Prevention:** Implement a rate limiter (e.g., using `os.clock()`) *before* the function can yield. If the request is rate-limited, fail securely and silently to prevent log-flooding, and ensure tracking tables are cleaned up on `PlayerRemoving`.
