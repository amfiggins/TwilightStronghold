...
## 2024-05-25 - Validation Bypass via NaN and Inf
**Vulnerability:** Core currency and inventory modification functions (`AddCurrency`, `AddItem`, `RemoveItem`) allowed `NaN` and `math.huge` values because `typeof(NaN)` and `typeof(math.huge)` evaluate to `"number"`, and `NaN <= 0` evaluates to `false`. This allowed malicious actors to corrupt inventories and bypass validation logic.
**Learning:** Checking `quantity <= 0` is insufficient in Luau due to floating-point math properties where `NaN` fails inequality checks.
**Prevention:** Explicitly check `x ~= x` (for NaN) and `x == math.huge` (for Infinity) in numeric validators, or explicitly use `x > 0` and complement it with finite number checks.
