...
## 2024-05-28 - DoS via Log-Flooding in Un-rate-limited Matchmaking Event
**Vulnerability:** The `JoinQueue` RemoteEvent in `src/server/MatchmakingService.lua` lacked a server-side rate limit. Malicious clients could spam requests to this event, causing the server to log messages excessively (e.g., `print(string.format("[Matchmaking] %s joined queue..."))`), leading to a Denial-of-Service (DoS) condition.
**Learning:** Even endpoints that have internal guards (like `if queueSet[player] then return end`) can be exploited to cause server degradation through intentional error generation or excessive state changes if they lack an explicit rate limit.
**Prevention:** All server-facing RemoteEvent handlers must implement a mandatory rate limiter (e.g., using `os.clock()`). When rate-limiting is enforced, the function should return silently rather than logging warnings. Additionally, any user-keyed tracking structures must bind to the `PlayerRemoving` event to ensure proper garbage collection of user data upon disconnect.
