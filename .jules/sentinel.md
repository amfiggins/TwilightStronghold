...
## 2024-05-26 - DoS Vulnerability via Unbounded Thread Creation in Yielding RemoteFunctions
**Vulnerability:** The `GetPlayerData` RemoteFunction lacked rate-limiting, allowing malicious clients to spam requests. Because the server yields (`task.wait()`) while polling for data, concurrent requests spawn unbounded threads and maintain active connections, leading to server resource exhaustion and a DoS condition.
**Learning:** Polling loops inside RemoteFunctions are highly dangerous without strict rate-limiting because clients control the invocation frequency. A flood of requests that yield indefinitely will quickly consume all server thread resources.
**Prevention:** All server-facing yielding RemoteFunctions must implement a mandatory strict rate limiter to immediately reject (`return nil`) rapid successive requests. Also, connection to `Players.PlayerRemoving` should ensure cleanup of memory tracking such limits.
