...
## 2024-07-04 - NaN Coordinate Spoofing in Combat System
**Vulnerability:** The spatial distance check in `CombatSystem.lua` used standard comparison logic (`distSq > maxRange * maxRange`), allowing an attacker to bypass range limits by providing NaN coordinates from a compromised client. Any comparison with NaN evaluates to false.
**Learning:** Standard comparison operators are vulnerable to NaN coordinate spoofing. Attackers can exploit this to perform actions (like melee attacks) globally across the map.
**Prevention:** Always use inverted comparison logic (e.g., `not (distSq <= maxRange * maxRange)`) for spatial boundary and range checks to safely handle and block NaN coordinate values.
