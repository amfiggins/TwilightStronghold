...
## 2024-05-24 - DoS Vulnerability via Unbounded Thread Creation on RemoteFunctions
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` lacked a server-side rate limit and utilized a polling mechanism (`while not data... task.wait(0.1)`). Malicious clients could spam this endpoint, causing unbounded thread creation and server resource exhaustion, leading to a DoS condition.
**Learning:** RemoteFunctions that yield (e.g., waiting for data to load) are highly susceptible to DoS attacks if un-rate-limited. Each concurrent invocation spawns a new thread on the server. Furthermore, per-user tracking tables must be cleaned up on disconnect to avoid memory leaks.
**Prevention:** Always implement a strict cooldown (e.g., using `os.clock()`) on client-facing RemoteFunctions, especially those that yield. Always bind cleanup logic to `Players.PlayerRemoving` for any user-keyed tracking tables.
