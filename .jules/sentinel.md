...
## 2024-05-24 - Yielding RemoteFunction DoS Vulnerability
**Vulnerability:** The `GetPlayerData` RemoteFunction yielded for up to 5 seconds per request without a rate limit, allowing malicious clients to exhaust server resources (thread exhaustion/memory) via continuous invocation spam.
...
**Prevention:** Implement server-side cooldown tracking tables (e.g., `lastGetDataTimes`) combined with explicit garbage collection during `Players.PlayerRemoving` to enforce rate limits and prevent DoS attacks on yielding RemoteFunctions.
