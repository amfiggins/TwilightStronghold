...
## 2024-05-24 - Rate Limiting RemoteFunctions to Prevent Thread Exhaustion DoS
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` polled internally (yielding via `task.wait()`) for up to 5 seconds without client-side or server-side rate limits. A malicious client could rapidly spam this RemoteFunction, causing the server to spawn an unbounded number of yielded threads, ultimately leading to thread exhaustion and a Denial of Service (DoS).
**Learning:** Asynchronous or yielding RemoteFunctions are highly susceptible to DoS attacks because each invocation spawns a new thread on the server. If the thread yields (e.g., waiting for data to load), memory and thread limits can be quickly exhausted by a simple while-loop exploit on the client.
**Prevention:** All RemoteFunctions and RemoteEvents must enforce strict, explicit server-side rate limiting (e.g., using a `lastRequestTimes` table). Additionally, rate-limiting tables must always have their entries cleared upon `Players.PlayerRemoving` to prevent memory leaks for disconnected users.
