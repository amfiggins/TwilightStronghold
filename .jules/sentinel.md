...
## 2024-05-24 - Missing NaN and math.huge Validation
**Vulnerability:** Numerical inputs allowed NaN and math.huge to bypass validation, potentially corrupting data or granting infinite items.
**Learning:** In Luau, NaN and math.huge pass numeric type checks and can bypass <= 0 checks.
**Prevention:** Always explicitly check for NaN (x ~= x) and math.huge (x == math.huge) when validating quantities.
