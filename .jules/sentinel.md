...
## 2026-05-07 - Numeric Validation Bypass in Roblox Luau
**Vulnerability:** Numerical inputs could bypass validation by supplying NaN or math.huge.
**Learning:** In Luau, NaN <= 0 is false, and math.huge <= 0 is false. This bypasses simple `amount <= 0` checks, leading to data corruption or item duplication.
**Prevention:** Always validate against NaN (`amount ~= amount`) and Infinity (`amount == math.huge`) alongside normal bounds checking.
