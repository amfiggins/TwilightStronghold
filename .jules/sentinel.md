...
## 2024-05-26 - Thread Exhaustion DoS via Un-rate-limited OnServerInvoke
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` contained a yielding loop (`task.wait(0.1)`) but lacked a rate limit check before entering the loop. Malicious clients could span requests, causing the server to suspend an unbounded number of threads, resulting in Thread Exhaustion and a server crash (DoS).
**Learning:** Any client-facing `RemoteFunction.OnServerInvoke` that yields (e.g., polling for data, `task.wait`, or web calls) is an extreme DoS vector if not immediately gated. Returning values or tracking time within the loop does not prevent the initial thread from being spawned and suspended.
**Prevention:** Always place a strict, non-yielding rate limit (e.g., using `os.clock()`) at the very beginning of any `OnServerInvoke` callback before any yielding operations can occur. Clear the rate limit tracking tables on `Players.PlayerRemoving` to prevent memory leaks.
