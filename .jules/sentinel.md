...
## 2024-05-26 - DoS Vulnerability in Yielding RemoteFunctions
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` lacked server-side rate limiting. Malicious clients could spam this endpoint causing unbounded thread creation (`task.wait` loops checking data presence), exhausting server resources and causing a Denial of Service (DoS).
**Learning:** Yielding RemoteFunctions (like polling loops or wait mechanisms) must have explicit server-side rate limits to prevent malicious clients from spamming requests and exhausting server resources via unbounded thread creation.
**Prevention:** Always implement a cooldown check (e.g., `os.clock() - lastRequest < COOLDOWN`) for client-facing RemoteFunctions. Return `nil` (or a generic failure response) securely and silently when rate-limited. Ensure tracking variables like `lastRequest` are explicitly cleaned up on `Players.PlayerRemoving` to prevent memory leaks over long-running server sessions.
