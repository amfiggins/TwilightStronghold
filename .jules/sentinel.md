...
## 2024-05-28 - DoS Vulnerability via Yielding RemoteFunction Polling
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` lacked server-side rate limiting. Malicious clients could spam requests to this endpoint, spawning infinite yielding loops (`while not data ... task.wait(0.1)`) that consumed server memory and thread execution limits, leading to Thread Exhaustion DoS.
**Learning:** Yielding `RemoteFunction.OnServerInvoke` callbacks must have explicit, non-yielding rate limits applied immediately at the entry point. A client can indefinitely stall a server thread if rate limits are missing.
**Prevention:** All server-facing RemoteFunction handlers that can yield must implement a mandatory rate limiter (e.g., using `os.clock()`). When rate-limiting is enforced, the function should return immediately. Additionally, per-user tracking tables must bind to the `PlayerRemoving` event to ensure proper garbage collection.
