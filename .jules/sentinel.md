...
## 2024-05-26 - Thread Exhaustion DoS via Un-rate-limited RemoteFunction Yielding
**Vulnerability:** The `GetPlayerData` RemoteFunction in `PlayerDataHandler.lua` contained a `while` loop that polled data with `task.wait()`, yielding the thread for up to 5 seconds. Because it lacked a rate limit, a malicious client could spam `InvokeServer()`, spinning up thousands of yielding threads and quickly exhausting the server's task scheduler capacity (Thread Exhaustion DoS).
**Learning:** Any RemoteFunction or RemoteEvent handler that yields (e.g., using `task.wait`, `GetAsync`, or `WaitForChild`) is highly vulnerable to DoS attacks if un-gated. The server will rapidly run out of memory or processing time trying to manage the concurrent yielded coroutines.
**Prevention:** Always implement strict, immediate, non-yielding rate limits (e.g., `os.clock()`) at the very top of any RemoteFunction or RemoteEvent handler *before* any yielding operations occur.
