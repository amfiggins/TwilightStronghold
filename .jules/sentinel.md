...
## 2024-05-24 - Rate Limiting Yielding Polling Loops
**Vulnerability:** The GetPlayerData RemoteFunction in src/server/PlayerDataHandler.lua lacks rate limiting and contains a yielding polling loop, a malicious client could spam InvokeServer() calls to create unbounded yielding threads, consuming server memory and bandwidth (DoS vulnerability).
**Learning:** Yielding RemoteFunctions must have explicit server-side rate limiting to prevent malicious clients from spamming requests and exhausting server resources via unbounded thread creation.
**Prevention:** Implement rate limiting using a lastGetDataTimes tracking table and a GET_DATA_COOLDOWN constant. Check this in OnServerInvoke and silently return nil if the client requests too frequently. Also update OnPlayerRemoving to clear the player's entry from the tracking table to prevent memory leaks.
