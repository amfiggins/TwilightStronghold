...
## 2024-05-25 - Thread Exhaustion DoS via Yielding RemoteFunctions
**Vulnerability:** The `GetPlayerData` RemoteFunction yielded (using `task.wait`) for up to 5 seconds without an immediate rate limit check. Attackers could spam this endpoint to exhaust server thread limits and cause a Denial of Service.
**Learning:** Any RemoteFunction that can yield execution (poll, wait, network call) must implement a strict, immediate, non-yielding rate limit (like `os.clock()`) before yielding.
**Prevention:** Track request times and enforce a strict cooldown per user before invoking any yielding operations inside `OnServerInvoke`. Clean up memory on `PlayerRemoving`.
