...
## 2024-05-01 - Fix Thread Exhaustion DoS in GetPlayerData
**Vulnerability:** The yielding `RemoteFunction.OnServerInvoke` callback in `PlayerDataHandler.lua` had no rate limiting, allowing malicious clients to spam requests and exhaust server threads.
**Learning:** Yielding operations within RemoteFunction callbacks must be protected from spam, as each invocation holds a thread.
**Prevention:** Implement explicit, non-yielding rate limits (using `os.clock()`) immediately upon invocation for any yielding RemoteFunction callbacks.
