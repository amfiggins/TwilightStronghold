...
## 2024-05-26 - DoS via Thread Exhaustion in RemoteFunctions
**Vulnerability:** The `GetPlayerData.OnServerInvoke` RemoteFunction lacked immediate rate limiting before a yielding polling loop (`task.wait`). This allowed malicious clients to spam the endpoint, suspending an unbounded number of server threads and causing a Denial-of-Service (DoS) condition.
**Learning:** Yielding `RemoteFunction.OnServerInvoke` callbacks must have explicit, non-yielding rate limits applied immediately at the entry point. A client can trigger thread exhaustion long before any internal timeouts resolve.
**Prevention:** Always enforce a cooldown check (e.g., using `os.clock()`) as the very first operation in any client-facing yielding endpoint. Return silently if rate-limited to avoid log flooding. Ensure user tracking tables are explicitly cleared on `Players.PlayerRemoving`.
