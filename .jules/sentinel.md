...
## 2024-04-15 - Thread Exhaustion DoS via Yielding RemoteFunctions
**Vulnerability:** The `GetPlayerData` `RemoteFunction.OnServerInvoke` callback contained a polling yield (`while not data do task.wait() end`) without immediate rate limiting. Malicious clients could invoke this function rapidly, exhausting server thread pools before any data resolved.
**Learning:** In Roblox Luau, yielding `RemoteFunction.OnServerInvoke` callbacks must implement explicit, non-yielding rate limits *immediately* upon invocation to prevent Thread Exhaustion DoS attacks from malicious clients.
**Prevention:** Always use `os.clock()` to enforce an immediate rate limit at the start of any `OnServerInvoke` handler that yields. Clean up the tracking tables in `PlayerRemoving`.
