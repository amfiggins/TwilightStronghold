...
## 2024-05-25 - Missing Rate Limit on Yielding RemoteFunction (DoS)
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` lacked a server-side rate limit. Malicious clients could spam `InvokeServer()` calls, spawning numerous threads that enter a `while` loop (polling for data). This unbounded thread creation and polling loop could consume significant server resources, leading to a Denial of Service (DoS).
**Learning:** RemoteFunctions that yield or poll (e.g., using `task.wait()`) are particularly dangerous if not rate-limited, as an attacker can easily exhaust server thread limits or CPU time by repeatedly invoking the function.
**Prevention:** Always implement a strict cooldown check (e.g., `now - lastGet < COOLDOWN`) at the very beginning of any `OnServerInvoke` handler, especially those that yield or perform heavy computation. Ensure the tracking tables are properly cleaned up on `Players.PlayerRemoving`.
