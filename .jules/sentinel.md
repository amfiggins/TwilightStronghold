...
## 2024-05-27 - NaN Spatial Validation Bypass
**Vulnerability:** Distance checks using `distSq > MAX * MAX` were bypassed by spoofed `NaN` coordinates, because any relational comparison against `NaN` evaluates to `false`.
**Learning:** `NaN` values bypass standard numerical comparisons, and can allow exploiters to bypass spatial checks.
**Prevention:** Use inverted comparison logic (e.g., `not (distSq <= MAX * MAX)`) to ensure `NaN` values evaluate correctly to block the action.
