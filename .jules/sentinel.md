...
## 2024-05-25 - Thread Exhaustion DoS via RemoteFunction Polling
**Vulnerability:** The `GetPlayerData.OnServerInvoke` in `PlayerDataHandler.lua` polled `PlayerDataHandler.Get` with a `task.wait(0.1)` for up to 5 seconds if data wasn't immediately ready, but lacked rate limiting. A malicious client could spam this function, creating thousands of yielding threads and quickly exhausting the server's task scheduler or memory limit, leading to a DoS condition.
**Learning:** Yielding operations (like `task.wait`) inside `OnServerInvoke` are especially dangerous because the server holds the thread open while waiting. If clients can spam the request, they can perform a thread exhaustion attack.
**Prevention:** All yielding `OnServerInvoke` callbacks must implement non-yielding rate limiting immediately at the entry point to reject excessive requests before any yielding occurs.
