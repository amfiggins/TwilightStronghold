...
## 2024-05-26 - DoS Vulnerability in Un-rate-limited Yielding RemoteFunctions
**Vulnerability:** The `GetPlayerData` RemoteFunction in `src/server/PlayerDataHandler.lua` lacked a server-side rate limit. Malicious clients could spam requests to this yielding endpoint (which polls using `task.wait(0.1)` for up to 5 seconds), leading to an unbounded number of polling threads on the server. This massive thread creation consumes significant server memory and processing power, creating a Denial-of-Service (DoS) condition.
**Learning:** Yielding RemoteFunctions, especially those that implement polling or wait mechanisms, are highly susceptible to DoS attacks if un-gated. A single malicious client can quickly exhaust server resources by initiating thousands of overlapping long-running requests.
**Prevention:** All server-facing RemoteFunctions that yield or perform asynchronous operations must implement a strict mandatory rate limiter (e.g., using `os.clock()`). Requests made within the cooldown period must be rejected silently (`return nil` or an error response) without yielding or logging. Additionally, rate-limiting tracking tables must connect to `Players.PlayerRemoving` to clear entries upon disconnect, preventing memory leaks over long sessions.
