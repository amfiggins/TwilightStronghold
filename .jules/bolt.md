...
## 2024-05-27 - Conditionally Gate UI Updates in Hot Loops
**Learning:** Unconditionally updating UI properties or calling `TweenService:Create` inside high-frequency per-frame loops (like `RenderStepped`) causes severe garbage collection and Lua-C boundary overhead, even if the value hasn't actually changed.
**Action:** Always cache the previous state value and conditionally gate UI updates so they only execute when the value actually changes.
