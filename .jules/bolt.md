...
## 2024-06-15 - String Allocation in Render Loops
**Learning:** In Roblox Luau, string allocation (e.g., via `string.format`) and UI `Text` property assignments within high-frequency loops (like `RenderStepped`) cause unnecessary overhead.
**Action:** Optimize by caching the underlying value (e.g., integer seconds) and only updating the string when the value changes.
