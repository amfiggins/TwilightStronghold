...
## 2024-05-27 - UI Update Caching in High-Frequency Loops
**Learning:** In Roblox Luau, unconditionally calling `TweenService:Create` or setting properties like `.Text` inside high-frequency per-frame loops (like `RenderStepped`) causes severe garbage collection and Lua-C boundary overhead, even when the visual state has not changed.
