...
## 2024-05-28 - Avoid Tween Creation in High-Frequency Loops
**Learning:** Unconditionally calling `TweenService:Create` and instantiating `TweenInfo` inside high-frequency loops (like `RunService.RenderStepped`) causes severe garbage collection overhead.
**Action:** Cache `TweenInfo` objects statically, maintain previous state values in a weak-keyed dictionary, and only conditionally create and play new Tweens when the underlying visual state actually changes.
