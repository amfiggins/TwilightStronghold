...
## 2024-05-24 - Unnecessary allocations in high-frequency loops
**Learning:** Calling `TweenService:Create` or updating text strings unconditionally inside `RunService.RenderStepped` causes severe garbage collection and Lua-C boundary overhead, as they create objects/strings every frame.
**Action:** Always cache previous state values outside the frame loop and conditionally gate UI updates so they only execute when the value actually changes.
