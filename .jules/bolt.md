...
## 2024-07-08 - Unconditional Tween Creation in RenderStepped
**Learning:** Unconditionally calling `TweenService:Create` or allocating strings for TextLabel updates inside high-frequency loops (like `RunService.RenderStepped`) causes severe garbage collection overhead.
**Action:** Always cache previous state values and only conditionally update strings or create new Tweens when the underlying UI state actually changes.
