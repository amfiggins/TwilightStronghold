...
## 2024-07-25 - Avoid Unconditional Tweens in RenderStepped
**Learning:** In Roblox Luau, unconditionally calling `TweenService:Create` inside high-frequency loops (like `RunService.RenderStepped`) causes severe garbage collection overhead.
**Action:** Always cache previous state values (e.g., using a weak-keyed metatable for Instances) and only conditionally create new Tweens when the underlying visual state actually changes. Cache `TweenInfo` objects as well when they are static.
