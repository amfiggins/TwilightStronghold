...
## 2024-05-30 - Tween Allocation Overhead
**Learning:** Calling `TweenService:Create` inside a `RenderStepped` loop (e.g., for continuous health bar tracking) allocates a new Tween object every frame, leading to severe garbage collection pressure and stuttering.
**Action:** Always maintain a cache of previous target values and only call `TweenService:Create` when the target value actually changes.
