...
## 2024-05-24 - Optimize RenderStepped UI updates in SurvivalHUD
**Learning:** Unconditionally calling `TweenService:Create` and updating string properties inside high-frequency per-frame loops (like `RenderStepped`) causes severe garbage collection and Lua-C boundary overhead.
**Action:** Always optimize by caching previous state values and conditionally gating UI updates so they only execute when the value actually changes.
