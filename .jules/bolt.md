...
## 2024-06-03 - Avoid Unconditional UI Updates in RenderStepped
**Learning:** Unconditionally calling `TweenService:Create` or formatting string properties inside high-frequency per-frame loops like `RenderStepped` causes severe garbage collection and Lua-C boundary overhead, even if the value hasn't visually changed.
**Action:** Always cache the last known value (or target UI state) and conditionally gate Tween creation and string manipulation so they only execute when a change actually occurs.
