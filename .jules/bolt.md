...
## 2024-08-06 - RenderStepped UI Allocations
**Learning:** Unconditionally calling `TweenService:Create`, formatting strings (e.g. `string.format`), or assigning UI `Text` properties inside high-frequency loops like `RunService.RenderStepped` causes severe garbage collection overhead.
**Action:** Always cache previous state values (like health ratio or integer timer seconds) and only trigger UI updates or Tween creations when the underlying state actually changes. Cache static `TweenInfo` objects.
