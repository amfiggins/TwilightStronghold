...
**Learning:** Unconditionally calling `TweenService:Create` or `string.format` (and assigning UI `Text` properties) inside high-frequency loops like `RunService.RenderStepped` causes severe garbage collection overhead.
**Action:** Optimize by caching previous state values (using a weak-keyed table for UI instances to prevent leaks) and only generating new strings or Tweens when the underlying value actually changes. Cache static `TweenInfo` objects as well.
