...
## 2024-07-01 - Prevent high-frequency Tween creation in RenderStepped
**Learning:** Unconditionally calling `TweenService:Create` or updating string properties inside high-frequency per-frame loops like `RunService.RenderStepped` causes severe garbage collection and Lua-to-C boundary overhead.
**Action:** Always cache previous state values and conditionally gate UI updates so they only execute when the value actually changes.
