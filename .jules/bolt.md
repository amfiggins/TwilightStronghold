...
## 2024-06-08 - Conditionally Update UI in RenderStepped Loops
**Learning:** Unconditionally calling `TweenService:Create` or updating string properties inside high-frequency loops like `RenderStepped` creates severe garbage collection overhead and Lua-C boundary crossings, impacting frame rates and causing stuttering.
**Action:** Always cache the previous state values and conditionally gate UI updates so they only execute when the value actually changes.
