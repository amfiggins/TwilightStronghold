...
## 2026-12-06 - Dual-Input Hover State Management
**Learning:** When implementing custom focus states for UI elements using both Mouse (MouseEnter/MouseLeave) and Gamepad/Keyboard (SelectionGained/SelectionLost) events, users mixing input methods can cause stuck visual highlighting if states aren't tracked independently.
**Action:** Independently track 'isHovered' and 'isSelected' boolean states and calculate the final visual state by checking if either is true using the `condition and A or B` pattern.
