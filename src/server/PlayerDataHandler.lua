...
-- Rate Limiting cache for GetPlayerData
...
function PlayerDataHandler.Init()
    ...
    GetPlayerData.OnServerInvoke = function(player)
        local now = os.clock()
        local lastGet = lastGetDataTimes[player.UserId] or 0
        if (now - lastGet) < GET_DATA_COOLDOWN then
            warn(string.format("[Data] %s is rate limited from GetPlayerData", player.Name))
            return nil
        end
