...
function PlayerDataHandler.Init()
    ...
    GetPlayerData.OnServerInvoke = function(player)
        ...
        local now = os.clock()
        local lastReq = lastGetDataTimes[player.UserId] or 0
        if (now - lastReq) < GET_DATA_COOLDOWN then
            warn(string.format("[Data] Rate limited GetPlayerData from %s", player.Name))
            return nil
        end
