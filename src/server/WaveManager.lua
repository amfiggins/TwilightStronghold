...
local function findNearestPlayer(position)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if character and character.PrimaryPart then
            local diff = character.PrimaryPart.Position - position
            local distSq = diff:Dot(diff) -- Optimization: use squared distance to avoid sqrt
...
function WaveManager.SpawnEnemy(difficulty)
    ...
    task.spawn(function()
        ...
        -- Optimization: Pre-calculate RaycastParams for LOS checks
        ...
        while enemy.Parent and humanoid and humanoid.Health > 0 do
            ...
            if targetPlayer and targetPlayer.Character and targetPlayer.Character.PrimaryPart then
                ...
                -- Optimization: Short-range Raycast to avoid expensive ComputeAsync
                if distSq < 900 then -- 30 studs squared (30^2 = 900)
                    local rayResult = workspace:Raycast(rootPart.Position, diff, raycastParams)

                    if not rayResult then
                        hasLOS = true
                    elseif rayResult.Instance:IsDescendantOf(targetPlayer.Character) then
                        hasLOS = true
                    end
                end
