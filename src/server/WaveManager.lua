...
local cachedPlayers = {} -- Cached player list for fast nearest-player lookup

function WaveManager.Init()
    print("[WaveManager] Initializing...")

    -- Cache current players
    ...
    Players.PlayerRemoving:Connect(function(player)
        local idx = table.find(cachedPlayers, player)
        if idx then
            table.remove(cachedPlayers, idx)
        end
    end)
