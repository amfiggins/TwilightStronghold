...
refreshTooltip.Position = UDim2.new(1, 0, -1, -5) -- Above the container
...
local function populateLoadout()
    ...
    local info = TweenInfo.new(1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1)
    refreshTween = TweenService:Create(refreshBtn, info, { Rotation = 360 })
    refreshTween:Play()
    ...
    if not success then
        ...
        if refreshTween then refreshTween:Cancel() end
