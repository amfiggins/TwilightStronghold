...
local refreshIsHovered = false
local refreshIsSelected = false

local function updateRefreshState()
    local isActive = refreshIsHovered or refreshIsSelected
...
local function createButton(text, onClick, rarityColor, isEquipped, rarityName, subtext)
    ...
    local function updateState()
        ...
        if rarityLabel then
            rarityLabel.Visible = active
        end

        if isEquipped then
            -- Interactive feedback even for equipped items (Brighter Green)
            btn.BackgroundColor3 = active and Color3.fromRGB(40, 100, 40) or Color3.fromRGB(30, 80, 30)
        else
            btn.BackgroundColor3 = active and Color3.fromRGB(80, 80, 80) or Color3.fromRGB(60, 60, 60)
        end
    end
