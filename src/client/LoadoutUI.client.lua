...
local isRefreshHovered = false
local isRefreshSelected = false

local function updateRefreshState()
    local isActive = isRefreshHovered or isRefreshSelected
...
local function createButton(text, onClick, rarityColor, isEquipped, rarityName, subtext)
    ...
    local isBtnHovered = false
    local isBtnSelected = false

    local function updateState()
        local activeState = isBtnHovered or isBtnSelected
        -- Always show rarity on hover for scanability
        if rarityLabel then
            rarityLabel.Visible = activeState
        end

        if isEquipped then
            -- Interactive feedback even for equipped items (Brighter Green)
            btn.BackgroundColor3 = activeState and Color3.fromRGB(40, 100, 40) or Color3.fromRGB(30, 80, 30)
        else
            btn.BackgroundColor3 = activeState and Color3.fromRGB(80, 80, 80) or Color3.fromRGB(60, 60, 60)
        end
    end
