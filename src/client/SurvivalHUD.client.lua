...
-- ⚡ Bolt: Cache previous values to prevent unconditional TweenService:Create
-- and string allocation overhead on every RenderStepped frame
local lastHealthRatio = -1
local lastTimerString = ""
...
RunService.RenderStepped:Connect(function(dt)
    ...
    local newTimerString = formatTime(phaseState.timeRemaining)
    if newTimerString ~= lastTimerString then
        lastTimerString = newTimerString
        timerLabel.Text = newTimerString
    end
end)
