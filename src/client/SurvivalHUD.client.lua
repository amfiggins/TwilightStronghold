...
-- ── Update helpers ────────────────────────────────────────────────────────
local barStateCache = setmetatable({}, { __mode = "k" })
...
local function setBarFill(fill, ratio)
    ...
    -- ⚡ Bolt: Cache ratio to avoid creating Tweens every frame in RenderStepped
    if barStateCache[fill] == clamped then
        return
    end
    barStateCache[fill] = clamped
