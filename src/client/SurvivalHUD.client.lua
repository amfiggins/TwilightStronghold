...
RunService.RenderStepped:Connect(function(dt)
    ...
    if humanoid then
        ...
        if ratio ~= lastHealthRatio then
            -- ⚡ Bolt: Cache health ratio to avoid creating a new Tween on every frame
    ...
    local newText = formatTime(phaseState.timeRemaining)
    if newText ~= lastTimerText then
        -- ⚡ Bolt: Cache timer text to avoid setting the Text property on every frame
