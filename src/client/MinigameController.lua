...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    elseif input.UserInputType.Name:match("Gamepad") then
        instructionLabel.Text = "Hold <b>R2</b> to align"
    end
end)
...
function MinigameController.Init()
    ...
    instructionLabel.Text = "Hold to align"
...
function MinigameController.Start(callback)
    ...
    connection = RunService.RenderStepped:Connect(function(dt)
        ...
        -- Logic: Move Bar with Spacebar, Touch, Mouse, or Gamepad R2
        ...
        for _, pad in ipairs(UserInputService:GetConnectedGamepads()) do
            if UserInputService:IsGamepadButtonDown(pad, Enum.KeyCode.ButtonR2) then
                isInputActive = true
                break
            end
        end
