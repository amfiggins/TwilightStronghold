...
function MinigameController.Start(callback)
    ...
    connection = RunService.RenderStepped:Connect(function(dt)
        ...
        -- Logic: Move Bar with Spacebar, Mouse Click/Touch, or Gamepad RT
        local spaceDown = UserInputService:IsKeyDown(Enum.KeyCode.Space)
        local clickDown = UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)

        -- Gamepad requires both UserInputType and KeyCode
        local padDown = false
        pcall(function()
            padDown = UserInputService:IsGamepadButtonDown(Enum.UserInputType.Gamepad1, Enum.KeyCode.ButtonR2)
        end)

        if spaceDown or clickDown or padDown then
